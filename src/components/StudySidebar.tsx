    ...
    <aside className="hidden w-56 shrink-0 lg:block">
      ...
      <nav className="space-y-4">
        {data.parts.map((part) => {
          ...
          return (
            <div key={part}>
              <p className="mb-1 text-sm font-medium text-foreground">{shortPartLabel(part)}</p>
                  ...
                  return (
                    <li key={section}>
                      <Link
                        to="/study"
                        search={(prev) =>
                          mergeStudySearch(prev, { part, section, cardId: undefined })
                        }
                      ...
                      >
                        ...
                        <span className="text-xs">{known}/{sectionCards.length} · {pct}%</span>
                      </Link>
                    </li>
                  );
                })}
...
export function computeStats(
  ...
  const review = cards.filter((c) => progress[String(c.id)] === "review").length;
  const learning = cards.filter((c) => progress[String(c.id)] === "learning").length;
